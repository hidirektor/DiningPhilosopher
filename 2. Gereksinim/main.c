...
void cataliAl(filozofYapi *filozofYapiP) {
    ...
    // 2. gereksinim için çatalların ikisininde boş olup olmamasına göre çatalları alıyor.
    if(catalP->kilit[filozofYapiP->filozofID] == NULL && catalP->kilit[(filozofYapiP->filozofID + 1) % filozofSayisi] == NULL) {
